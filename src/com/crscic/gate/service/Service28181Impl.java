...
public class Service28181Impl implements IService
{
	...
	public void startService(IConfig config, IConnector senderConnector, IConnector recvConnector)
	{
		...
		properties.setProperty("local.port", config.getProperty("me.port"));
		...
		try
		{
			...
			SipProvider senderSipProvider = sender.getSipProvider();
			ClientTransaction inviteTid = senderSipProvider.getNewClientTransaction(request);
